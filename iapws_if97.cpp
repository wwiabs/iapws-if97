...
//p5 kJ/kg
const double  specific_gas_constant = 0.461526e3;
...
//p10
const double  R1_back_T_ph[20][3] = {
...
//p11
const double  R1_back_T_ps[20][3] = {
...
//p13
const double  R2_base_ideal[9][2] = {
...
//p22
const double  R2a_back_T_ph[34][3] = {
...
//p23
const double  R2b_back_T_ph[38][3] = {
...
//p24
const double R2c_back_T_ph[23][3] = {
...
//p26
const double  R2a_back_T_ps[46][3] = {
...
//p27
const double  R2b_back_T_ps[44][3] = {
...
//p28
const double  R2c_back_T_ps[30][3] = {
...
//p152_book
const double viscosity_in_the_ideal_gas_limit[4]={
    0.167752e-1,
    0.220462e-1,
    0.6366564e-2,
    -0.241605e-2};
...
//p5  单位:K
//inline double IAPWS_IF97::B23_p_T(double T){ return  B23[0] + B23[1] * T + B23[2] * pow(T, 2); }

//p6
inline double B23_T_p(const double& p){ return  B23[3] + pow((p - B23[4]) / B23[2], 0.5); }

//p6  单位:Mpa,K
double gamma_R1(const double& pi,const double& tau)
{
	double sum = 0;
	for (int i = 0; i<34; i++)
	{
...
//p8  单位:Mpa,K
double gamma_tau_R1(const double& pi,const double& tau)
{
    double sum = 0;
    for (int i = 0; i<34; i++)
    {
...
//p8  单位:Mpa,K
double gamma_pi_R1(const double& pi, const double& tau)
{
    double sum = 0;
    for (int i = 0; i<34; i++)
    {
...
//p13  单位:Mpa,K
double gamma_ideal_R2(const double& pi,const double& tau)
{
    double sum = log(pi);
    for (int i = 0; i<9; i++)
    {
...
//p13 单位:Mpa,K
double gamma_res_R2(const double& pi, const double& tau)
{
    double sum = 0;
    for (int i = 0; i<43; i++)
    {
...
//p16 单位:Mpa
inline double gamma_pi_ideal_R2(const double& pi) { return 1 / pi; }

//p16  单位:Mpa,K
double gamma_pi_res_R2(const double& pi, const double& tau)
{
    double sum = 0;
    for (int i = 0; i<43; i++)
    {
...
//p16 单位:K
double gamma_tau_ideal_R2(const double& tau)
{
    double sum = 0;
    for (int i = 0; i<9; i++)
    {
...
//p16  单位:Mpa,K
double gamma_tau_res_R2(const double& pi, const double& tau)
{
    double sum = 0;
    for (int i = 0; i<43; i++)
    {
...
//p21
inline double R2_Bbc_h_p(const double& p)
{
    return  B2bc_back[3] + pow((p - B2bc_back[4]) / B2bc_back[2], 0.5);
}

//p11_book
int region_pT(const double& p, const double& T)
{
...
//p37_book
int region_ph(const double& p, const double& h)
{
...
//p53_book
int region_ps(const double& p, const double& s)
{
...
double v_pT(const double& p, const double& T,int region )
{
    double result = 0;
    ...
    switch (region)
    {
        case 1:
        {
            ...
            double pi = p / 16.53;
            double tau = 1386 / T;
            result = pi * specific_gas_constant * T * gamma_pi_R1(pi, tau) / p / 1e6;
            break;
        }
        case 2:
        {
            //p15
            double tau = 540 / T;
            result = p*(gamma_pi_ideal_R2(p) + gamma_pi_res_R2(p, tau))* specific_gas_constant * T / p / 1e6;
            break;
        }
        default:
            result = 0;
...
double s_pT(const double& p, const double& T, int region)
{
    double result = 0;
    region = (region == 0) ? region_pT(p, T) : region;
    switch (region)
    {
        case 1:
        {
            ...
            double pi = p / 16.53;
            double tau = 1386 / T;
            result = specific_gas_constant * (tau * gamma_tau_R1(pi, tau) - gamma_R1(pi, tau)) / 1000;
            break;
        }
        case 2:
        {
            ...
            result = specific_gas_constant * (tau * (gamma_tau_ideal_R2(tau) + gamma_tau_res_R2(p, tau))
                - gamma_ideal_R2(p, tau) - gamma_res_R2(p, tau)) / 1000;
            break;
        }
        default:
            result = 0;
...
double h_pT(const double& p, const double& T, int region)
{
    double result = 0;
    region = (region == 0) ? region_pT(p, T) : region;
    switch (region)
    {
        case 1:
        {
            ...
            double pi = p / 16.53;
            double tau = 1386 / T;
            result = tau * specific_gas_constant * T * gamma_tau_R1(pi, tau) / 1000;
            break;
        }
        case 2:
        {
            //p15
            double tau = 540 / T;
            result = specific_gas_constant * T * tau * (gamma_tau_ideal_R2(tau) + gamma_tau_res_R2(p, tau)) / 1000;
            break;
        }
        default:
            result = 0;
...
double Ts_p(const double& p)
{
	if (p>=611.213e-6&&p<=22.064){
		...
		double E = pow(beta, 2) + sat[2] * beta + sat[5];
		double F = sat[0] * pow(beta, 2) + sat[3] * beta + sat[6];
		double G = sat[1] * pow(beta, 2) + sat[4] * beta + sat[7];
		double D = 2 * G / (-pow(pow(F, 2) - 4 * E * G, 0.5) - F);
		return (sat[9] + D - pow(pow(sat[9] + D, 2) - 4 * (sat[8] + sat[9] * D), 0.5)) / 2;
	}
	else return 0;
}
...
double ps_T(const double& T)
{
	if (T>=273.15&&T<=647.096){
		...
		double A = pow(theta, 2) + sat[0] * theta + sat[1];
		double B = sat[2] * pow(theta, 2) + sat[3] * theta + sat[4];
		double C = sat[5] * pow(theta, 2) + sat[6] * theta + sat[7];
		return pow(2 * C / (pow(pow(B, 2) - 4 * A * C, 0.5) - B), 4);
	}
	else return 0;
}
//p152 book
double viscosity_ideal(const double& theta)
{
    double result=0;
	for (int i = 0; i < 4; i++)   result += viscosity_in_the_ideal_gas_limit[i] * pow(theta, -i);
    return pow(theta,0.5)/result;
}

//p152 book
double viscosity_second(const double& delta,const double& theta)
{
    double result=0;
	for (int i = 0; i < 21; i++) result += viscosity_2[i][2] * pow(delta - 1, viscosity_2[i][0])*pow(1 / theta - 1, viscosity_2[i][1]);
